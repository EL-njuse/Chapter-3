...
public class PPTtalk : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isEntered && Manger.instance.dialogueBox.activeInHierarchy == false && hasBeenShow == 0)
        {
            Invoke("saySomething", delayTime);
            hasBeenShow++;
        }

        if (ifNext && Manger.instance.ifFinished && hasBeenShow == 1)
        {
